 ...
 */
static int registed = app_autocall_function( &wifimon_app_setup, APP_PRIO( APP_GROUP_NETWORK, 0 ) );           /** @brief app autocall function */
/*
...
static void enter_wifimon_app_event_cb( lv_obj_t * obj, lv_event_t event ) {
    switch( event ) {
        case( LV_EVENT_CLICKED ):       app_hide_indicator( wifimon_app );
                                        mainbar_jump_to_tilenumber( wifimon_app_main_tile_num, LV_ANIM_OFF, true );
                                        break;
    }    
}
