 ...
 */
static int registed = app_autocall_function( &mqtt_control_app_setup, APP_PRIO( APP_GROUP_MEDIA, 4 ) );           /** @brief app autocall function */
...
static void enter_mqtt_control_event_cb( lv_obj_t * obj, lv_event_t event ) {
    switch( event ) {
        case( LV_EVENT_CLICKED ):       mainbar_jump_to_tilenumber( mqtt_control_main_tile_num, LV_ANIM_OFF, true );
                                        app_hide_indicator( mqtt_control_app );
                                        mqtt_start();
                                        break;
    }    
